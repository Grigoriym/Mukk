#!/usr/bin/env bash
#
# Fails a commit that weakens this repo's lint/CI gate without saying so out loud.
#
# The gate (detekt, CI) constrains the code a session writes. Nothing constrains a session from
# widening it so its own change passes. This does not prevent that — it makes it impossible to
# do quietly: a commit that trips a wire has to carry a
#
...
# gradle/libs.versions.toml is on the list above, but it's also where every ordinary dependency
# bump (kotlin, koin, exposed, gst1-java-core, ...) lands — those don't decide whether anything
# passes. Only the detekt/composeRules version keys actually govern the lint gate, so the wire
# only fires when one of them is what changed.
GATE_VERSION_KEYS='^[+-](detekt|composeRules)[[:space:]]*='
...
# side brought in, not something *this* commit introduced.
      ...
      case "$wire" in
        gradle/libs.versions.toml)
          if [ "$file" = "$wire" ] && libs_versions_touches_gate "$base" "$sha"; then
            reasons+=("touches $file (a gate-relevant version: detekt/composeRules)")
          fi
          ;;
